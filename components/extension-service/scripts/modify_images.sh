...
SERVER_IMAGE_TAG=$(aws ecr describe-images --repository-name "$SERVER_REPO" --query "reverse(sort_by(imageDetails, &imagePushedAt))[:1]" | jq '.[]|.imageTags|.[]|select(contains("commit"))')
MANAGER_IMAGE_TAG=$(aws ecr describe-images --repository-name "$MANAGER_REPO" --query "reverse(sort_by(imageDetails, &imagePushedAt))[:1]" | jq '.[]|.imageTags|.[]|select(contains("commit"))')
BUILDER_IMAGE_TAG=$(aws ecr describe-images --repository-name "$BUILDER_REPO" --query "reverse(sort_by(imageDetails, &imagePushedAt))[:1]" | jq '.[]|.imageTags|.[]|select(contains("commit"))')
